...
class ConsultationIn(BaseModel):
    ...
    email: EmailStr
    phone: str
    consultation_type: str  # bridal / diamond
    mode: str  # in-store / video
...
async def create_order(payload: OrderIn):
    """Creates an order record. Razorpay order creation would be plugged in here."""
    order_id = f"CNT-{uuid.uuid4().hex[:8].upper()}"
    doc = payload.model_dump()
    doc["id"] = str(uuid.uuid4())
    doc["order_number"] = order_id
